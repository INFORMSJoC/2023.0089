...
// the purpose of function CircleSiteConstuct() is to find the highest-energy points in the current configration S. 
...
void VacancySiteConstuct(Solution &S, Site VSites[], int MaxV, double r)
{
	...
	while(i < 5*N)
	{
	    ...
	    {
	    	int jiaodian = judin(V.x[0],V.x[1],numPH[hh],holepoint[hh]);  
	    	if(jiaodian%2) 
	    	{
	    		jud_inhole = 1;
	    		break;
		...
		cg_descent(V.x, 2, NULL, &UParm1, 1.e-8, myvalue_sub, mygrad_sub, NULL, NULL); // Local minimization
...
void TabuSearch(Solution &S, int SearchDepth, double rr)
{
 ...
 int best_v[2]; // best non-neighbor solution denoted by (v[0],v[1]), where v[0] denotes the nubmering of point to be moved and v[1] denotes the numbering of site to be occupyied. 
 int tabu_best_v[2] ; // best tabu-neighbor solution denoted by (v[0],v[1]), where v[0] denotes the nubmering of point to be moved and v[1] denotes the numbering of site to be occupyied.
 int CircleSelected; // the index of point to be moved. 
 ...
 while(NoImprove < SearchDepth && iter < 200 && 1.0*(clock()- starting_time)/CLOCKS_PER_SEC < Time_limit)
  {
	...
	MBH(CS, rr); // performs MBH procedure 
