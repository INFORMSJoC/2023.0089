	...
	if(hole == 'H') 
	{
		...
		while(1)
		{
			for(int l=0;l<=numPH[holnum];l++)
			{
				...
				{
					fscanf(fpRead,"%lf %lf\n",&pt1,&pt2);
					holepoint[holnum][2*l]   = pt1;   
					holepoint[holnum][2*l+1] = pt2;   
				}
	        }
...
void Inputing1(Solution &S, char *filename, int n)
{
    int i,j,num;
	FILE *fpRead;
	char buff[200];
...
void Out_results(double best , double ave,  double worst, int sr, double AvgTime, double deviation, char *filename, char *inputfile, int N)
{
    ...
    sprintf(buff,"%s",filename);
    fp = fopen(buff,"a+");
    fprintf(fp,"%s    %d   %.10f   %.10f   %.10f  %d  %e   %.2f\n",inputfile, N, 2*best, 2*ave, 2*worst, SR, deviation, AvgTime);
	fclose(fp);
}
